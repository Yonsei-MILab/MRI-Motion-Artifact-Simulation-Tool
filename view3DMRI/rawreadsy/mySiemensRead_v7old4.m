...
if ~reprocess
    ...
    if exist_full_raw_file && ~same_mySiemensRead_version
...
%% +++++ Procsessing ONLY MDH structure ++++++
...
% % tic
% % for i=1:524289
% %     a=fread(fid,mdh/2,'uint8');
% %     b=fread(fid,mdh/4,'uint16');
% % %     evm1=typecast(a(21:24),'uint32');
% % %     evm2=typecast(a(25:28),'uint32');
% % %     readMDHmatsome(fid);
% % %     fseek(fid,20,'cof');
% % %     mMDH(6:7)                 = fread(fid, 2, 'uint32');      % 20 + 2 * 4 = 28 
% % %     mMDH(8:23)                        = fread(fid, 16, 'uint16');      % 30 
% % %     fseek(fid,64,'cof');
% % %     mMDH(47)                             = fread(fid, 1, 'uint16');      % 124 + 2 = 126 
% % %     fseek(fid,2,'cof');
% % 
% % 
% %     fseek(fid,512*2*4,'cof');  % *4 -> float32
...
% % assignin('base','a',a);
% % assignin('base','b',b);
% % % assignin('base','evm1',evm1);
% % % assignin('base','evm2',evm2);
% % return;
% 
% ny = 0;
% nz = 0;
...
% disp('initialize raw cell matrix...')
% if donotAvg
%     raw = cell(ns,nc,na,ne,nr,nset,nphase);
%     avg_cell = cell(ns,nc,na,ne,nr,nset,nphase);
%     raw1zeros = complex(zeros(ny,nx,nz,'single'));
%     avg1zeros = zeros(ny,1,nz,'uint16');
%     for ls=1:ns
%         for le=1:ne
%             for lr=1:nr
%                 for lset=1:nset
%                     for lphase=1:nphase
%                         for lc=1:nc
%                             raw{ls,lc,la,le,lr,lset,lphase} = raw1zeros;
%                             avg_cell{ls,lc,la,le,lr,lset,lphase} = avg1zeros;
%                         end
%                     end
%                 end
%             end
...
%     end
% else
%     raw = cell(ns,nc,1,ne,nr,nset,nphase);
%     avg_cell = cell(ns,nc,1,ne,nr,nset,nphase);
%     raw1zeros = complex(zeros(ny,nx,nz,'single'));
%     avg1zeros = zeros(ny,1,nz,'uint16');    
%     for ls=1:ns
%         for le=1:ne
...
%                     for lphase=1:nphase
%                         for lc=1:nc
%                             raw{ls,lc,1,le,lr,lset,lphase} = raw1zeros;
%                             avg_cell{ls,lc,1,le,lr,lset,lphase} = avg1zeros;
%                         end
%                     end
%                 end
%             end
...
%             temp_navRT_image = temp_navRT_image(nav_ny/2,:);
        ...
        % image raw data
        % structure : {ns,nc,na,ne,nr,nset,nphase} [ny,nx,nz]
        % NOTE :
    else
        ...
        if cur_c ~= cp_coil_index    % did not include CP coil
            ...
            if ~donotAvg && (cur_a>1 || mMDH(MDH_aushFreePara1) == 302) 
                ...
                try
                    % --------- save acq index to average
                    avg_cell{cur_s,1,cur_e,cur_r,cur_set,cur_phase}...
                        {cur_y,cur_z,cur_c} = avg_cell{cur_s,1,cur_e,cur_r,cur_set,cur_phase}...
                        {cur_y,cur_z,cur_c} + 1;                    
                    % ------ add raw
                    % ADC data (temp) -> saved column vector
                    raw{cur_s,1,cur_e,cur_r,cur_set,cur_phase}...
                        {cur_y,cur_z,cur_c} = raw{cur_s,1,cur_e,cur_r,cur_set,cur_phase}...
                        {cur_y,cur_z,cur_c} + temp.';
                ...
                avg_cell{cur_s,cur_a,cur_e,cur_r,cur_set,cur_phase}...
                        {cur_y,cur_z,cur_c} = 1;                
                % save raw
                raw{cur_s,cur_a,cur_e,cur_r,cur_set,cur_phase}...
                        {cur_y,cur_z,cur_c} = temp;
            end     
...
%                 try
%                     % --------- save acq index to average
%                     avg_cell{cur_s,cur_c,1,cur_e,cur_r,cur_set,cur_phase}...
%                         (cur_y,1,cur_z) = avg_cell{cur_s,cur_c,1,cur_e,cur_r,cur_set,cur_phase}...
%                         (cur_y,1,cur_z) + 1;                    
%                     % ------ add raw
%                     % ADC data (temp) -> saved column vector
%                     raw{cur_s,cur_c,1,cur_e,cur_r,cur_set,cur_phase}...
%                         (cur_y,:,cur_z) = raw{cur_s,cur_c,1,cur_e,cur_r,cur_set,cur_phase}...
%                         (cur_y,:,cur_z) + temp.';
%                     
%                 catch %-- in case of first appeared (mMDH(MDH_aushFreePara1) == 302)
%                     
...
%             else % if donotAvg==1, sepatate acq
% 
%                 avg_cell{cur_s,cur_c,cur_a,cur_e,cur_r,cur_set,cur_phase}...
%                         (cur_y,1,cur_z) = 1;                
%                 % save raw
%                 raw{cur_s,cur_c,cur_a,cur_e,cur_r,cur_set,cur_phase}...
%                         (cur_y,:,cur_z) = temp;
%             end     
...
if ~isempty(projLine_raw_cell)
    ...
    [ny,nz,ns,na] = size(raw);
    [nx,nc,ne,nr,nset,nphase] = size(raw{1});
                            ...
                            for lset = 1:nset
                                for lphase = 1:nphase
                                    for la = 1:na
                                        ...
                                        if ~isempty(raw{ly,lz,ls,la})
                                            raw{ly,lz,ls,la}(:,lc,...
                                                le,lr,lset,lphase) = raw{ly,lz,ls,la}(:,lc,...
                                                le,lr,lset,lphase)/avg_cell{ly,lz,ls,la}(:,lc,...
                                                le,lr,lset,lphase);
                                        end
...
%% consider KyCenter & KzCenter
    ...
    if num_zero_padding>0
        temp_zeros_cell = cell(num_zero_padding,nz,ns,na);
    ...
    if nz>1
        ...
        % zero filling to locate CentreLine center
        if num_zero_padding>0
            temp_zeros_cell = cell(ny,num_zero_padding,ns,na);
            % NOTE : cell can be defined and concatenated with empty
            % element
...
%% save full raw data
    ...
    raw_info_text = strvcat(' ',...
        ...
        '---------------------------------------------',...
        'full_raw data structure : {ny,nz,ns,na} [nx,nc,ne,nr,nset,nphase]',...
        '    NOTE : this structure is most effective memory savings when considering',...
        '    zero-padding AND for double valued average factor',...
        ' ',...
...
%% seperate full-raw
        ...
        if available_phMEMSizeMByte < full_raw_FileSizeMByte/2
            ...
            % save seperate file
            if nz>1 % in 3D, to each partition
                for z=1:nz
                    if z<10
                        add0z = '00';
                    elseif z<100
                                    ...
                                    if make_3DwithCoil
                                        ...
                                        for z=1:nz
                                            ...
                                            for y=1:ny
                                                    ...
                                                    if is_full_raw
                                                        if ~isempty(raw{y,z,s,ia})
                                                            raw_part(y,:,z,:) = raw{y,z,s,ia}(:,:,e,r,i_set,iphase);
                                                        else
                                                            raw_part(y,:,z,:) = 0;
                                                        end
                                                    else
                                                        if ~isempty(raw_partition{y,1,s,ia})
                                                            raw_part(y,:,z,:) = raw_partition{y,1,s,ia}(:,:,e,r,i_set,iphase);
                                                        else
                                                            raw_part(y,:,z,:) = 0;
                                                        end
                                        ...
                                        for c = 1:nc
                                            ...
                                            for z=1:nz
                                                ...
                                                for y=1:ny
                                                    ...
                                                    if is_full_raw
                                                        if ~isempty(raw{y,z,s,ia})
                                                            raw_part(y,:,z) = raw{y,z,s,ia}(:,c,e,r,i_set,iphase);
                                                        else
                                                            raw_part(y,:,z) = 0;
                                                        end
                                                    else
                                                        if ~isempty(raw_partition{y,1,s,ia})
                                                            raw_part(y,:,z) = raw_partition{y,1,s,ia}(:,c,e,r,i_set,iphase);
                                                        else
                                                            raw_part(y,:,z) = 0;
                                                        end
                                                    end % end of 'if is_full_raw'
                        ...
                        for r = 1:nr
                            ...
                            for e = 1:ne
                                ...
                                for s = 1:ns
                                    ...
                                    if is_full_raw
                                        for y=1:ny
                                            if ~isempty(raw{y,1,s,ia})
                                                raw_part(y,:,:) = raw{y,1,s,ia}(:,:,e,r,i_set,iphase);
                                            else
                                                raw_part(y,:,:) = 0;
                                            end
                                        end
                                        ...
                                        for y=1:ny
                                            if ~isempty(raw_slice{y,1,1,ia})
                                                raw_part(y,:,:) = raw_slice{y,1,1,ia}(:,:,e,r,i_set,iphase);
                                            else
                                                raw_part(y,:,:) = 0;
                                            end
                                        end
