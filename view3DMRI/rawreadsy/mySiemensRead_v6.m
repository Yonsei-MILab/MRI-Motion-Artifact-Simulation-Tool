            ...
            if ~donotAvg && (cur_a>1 || sMDH.aushFreePara(1) == 302) 
                ...
                try
                    ...
                    raw{cur_y,cur_z}(:,cur_s,...
                        cur_e,cur_r,cur_set,cur_phase,cur_c) = raw{cur_y,cur_z}(:,cur_s,...
...
if ~isempty(projLine_raw_cell)
    ...
    [ny,nz,na] = size(raw);
    [nx,ns,ne,nr,nset,nphase,nc] = size(raw{1});
                            ...
                            for lset = 1:nset
                                for lphase = 1:nphase
                                    for la = 1:na
                                        ...
                                        if ~isempty(raw{ly,lz,la})
                                            raw{ly,lz,la}(:,ls,...
                                                le,lr,lset,lphase,lc) = raw{ly,lz,la}(:,ls,...
                                                le,lr,lset,lphase,lc)/avg_cell{ly,lz,la}(:,ls,...
                                                le,lr,lset,lphase,lc);
                                        end
...
%% consider KyCenter & KzCenter
    ...
    if num_zero_padding>0
        temp_zeros_cell = cell(num_zero_padding,nz,na);
    ...
    if nz>1
        ...
        % zero filling to locate CentreLine center
        if num_zero_padding>0
            temp_zeros_cell = cell(ny,num_zero_padding,na);
            % NOTE : cell can be defined and concatenated with empty
            % element
...
%% save full raw data
    ...
    raw_info_text = strvcat(' ',...
        ...
        '---------------------------------------------',...
        'full_raw data structure : {ny,nz,na} [nx,ns,ne,nr,nset,nphase,nc]',...
        '    NOTE : this structure is most effective memory savings when considering',...
        '    zero-padding AND for double valued average factor',...
        ' ',...
...
%% seperate full-raw
        ...
        full_raw_FileSizeMByte = nc*nz*ns*ny*nx*ne*nr*nset*nphase*na*8/1e6;
        fprintf(' - Full Raw data size = %g [MByte]\n\n',full_raw_FileSizeMByte)
        
        % if (not 64 bit system AND larger than 300 MByte) OR larger than 4 GByte
        if (isempty(strfind(computer,'64')) && full_raw_FileSizeMByte > 300) || full_raw_FileSizeMByte > 4000
                                ...
                                for s = 1:ns
                                    ...
                                    for c = 1:nc
                                        ...
                                        for y=1:ny
                                            ...
                                            if ~is_full_raw
                                                disp(['Loading seperated raw data file... ','raw_ky_',add0y,num2str(y)])
                                                load([pathnameSS,'Raw_Data_VB15/',filenameSS,...
                                                    '_raw_ky_',add0y,num2str(y),'.mat'])
                                            end
                                            ...
                                            for z=1:nz
                                                ...
                                                if make_3DwithCoil
                                                    ...
                                                    % coils -> 4D matrix
                                                    if is_full_raw
                                                        if ~isempty(raw{y,z,ia})
                                                            raw_part(y,:,z,c) = raw{y,z,ia}(:,s,e,r,i_set,iphase,c);
                                                        else
                                                            raw_part(y,:,z,c) = 0;
                                                        end
                                                    else
                                                        if ~isempty(raw_ky{1,z,ia})
                                                            raw_part(y,:,z,c) = raw_ky{1,z,ia}(:,s,e,r,i_set,iphase,c);
                                                        else
                                                            raw_part(y,:,z,c) = 0;
                                                        end
                                                    ...
                                                    % make 3D data without
                                                    % coils -> 3D matrix
                                                    if is_full_raw
                                                        if ~isempty(raw{y,z,ia})
                                                            raw_part(y,:,z) = raw{y,z,ia}(:,s,e,r,i_set,iphase,c);
                                                        else
                                                            raw_part(y,:,z) = 0;
                                                        end
                                                    else
                                                        if ~isempty(raw_ky{1,z,ia})
                                                            raw_part(y,:,z) = raw_ky{1,z,ia}(:,s,e,r,i_set,iphase,c);
                                                        else
                                                            raw_part(y,:,z) = 0;
                                                        end
                                                    end % end of 'if is_full_raw'
                            ...
                            for e = 1:ne
                                ...
                                for s = 1:ns
                                    ...
                                    for y=1:ny
                                        ...
                                        if is_full_raw
                                            for c=1:nc
                                                if ~isempty(raw{y,1,ia})
                                                    raw_part(y,:,c) = raw{y,1,ia}(:,s,e,r,i_set,iphase,c);
                                                else
                                                    raw_part(y,:,c) = 0;
                                                end
                                            end
                                            ...
                                            for c=1:nc
                                                if ~isempty(raw_ky{1,1,ia})
                                                    raw_part(y,:,c) = raw_ky{1,1,ia}(:,s,e,r,i_set,iphase,c);
                                                else
                                                    raw_part(y,:,c) = 0;
                                                end
                                            end
                                        end
                                    end % end of for y=1:ny
