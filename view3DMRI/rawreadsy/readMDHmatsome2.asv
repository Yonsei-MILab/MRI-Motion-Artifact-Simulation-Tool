...
%-- ¿Ã∞‘ »Œæ¿ ¥¿∏≤
a=fread(fid, 8, '*uint8');      % 20 + 2 * 4 = 28
mMDH(6) = double(typecast(a(1:4),'uint32'));
mMDH(7) = double(typecast(a(5:8),'uint32'));

% fread(fid, 2, 'uint32');      % 20 + 2 * 4 = 28
% mMDH(6:7)                 = fread(fid, 2, 'uint32');      % 20 + 2 * 4 = 28
% mMDH(7)                 = fread(fid, 1, 'uint32');

a=fread(fid, 32, '*uint8');      % 20 + 2 * 4 = 28
for i=1:16
    mMDH(7+i) = double(typecast(a(i:i+1),'uint16'));
end

% fread(fid, 16, 'uint16');      % 30
...
a=fread(fid, 2, '*uint8');      % 20 + 2 * 4 = 28
mMDH(47) = double(typecast(a(1:2),'uint16'));
% fread(fid, 1, 'uint16');      % 124 + 2 = 126
