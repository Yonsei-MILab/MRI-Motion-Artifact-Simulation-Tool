...
% 
% display information on a figure
% correct zero-padding error on raw data
% ---------- Last modified at 2009.12.07
% 
% Navigator image processing is done for 2D PACE
% fix codes on MDH processing to increase speed
% ---------- Last modified at 2009.12.08
%
% ---------- Futher work todo
% 
% bug fixed on averaging
% ---------- Last modified at 2010.01.08
%
% ---------- Futher work todo
% 
% bug fixed on resolution info.
% ---------- Last modified at 2010.04.16
%
% ---------- Futher work todo
% 
% file name info.
% ---------- Last modified at 2010.04.19
%
% ---------- Futher work todo
% 
% add option of 'donotSaveFullRaw' 'donotPadZeros' 'doFFTScaling'
% ---------- Last modified at 2010.06.01
% 
% remove memory check for other OS - seperate file will be not saved
% ---------- Last modified at 2010.06.10
% 
% change fft -> ifft,    i.e.,   raw -> ifft -> image
% ---------- Last modified at 2010.07.26
% 
% change ifft -> fft  in RT images
% ----- % ifft : spoil image (break rule) - found at 2010.09.08
% ---------- Last modified at 2010.09.08
% 
% 3D radial, eja_CODE data processing was included.
% global variable for checkbox were removed.
% ---------- Last modified at 2010.09.09
% 
% change ifft -> fft again,    i.e.,   raw -> fft -> image
% ifft shift 1 partition in 3D data
% ---------- Last modified at 2010.11.02
% 
% add index information during file saving (on info board)
% ---------- Last modified at 2010.11.11
% 
% add zero padding only for kz
...
% 
% j -> 1i
% ---------- Last modified at 2011.04.05
% 
% remove flickering info_board in batch process
% ---------- Last modified at 2011.04.07
% 
% fix bugs on MDH_SYNCDATA
...
local_text4print = sprintf('\nmySiemensRead%s()\n',mySiemensRead_version);
fprintf(local_text4print)
disp2infoboard(info_fig_handle,local_text4print)
...
% ------------ get checkbox (16) state -----------------------
make_comp_im = get(handles.checkbox_makecomp,'Value');
cut_RO_OS = get(handles.checkbox_cutreados,'Value');
donotSwapRO = get(handles.checkbox_donotswap,'Value');
reprocess = get(handles.checkbox_reprocess,'Value');
donotAvg = get(handles.checkbox_noAvg,'Value');
make_same_slice_DCM = get(handles.checkbox_sameSliceDCM,'Value');
make_3DwithCoil = get(handles.checkbox_make3DwithCoil,'Value');
useInterleaving = get(handles.checkbox_slice_interleave,'Value');
ignore_image_data = get(handles.checkbox_wo_ima,'Value');
donotSaveFullRaw = get(handles.checkbox_donotSaveFullRaw,'Value');
donotPadZeros = get(handles.checkbox_donotPadZeros,'Value');
doFFTScaling = get(handles.checkbox_doFFTScaling,'Value');
gSkipifft4im = get(handles.checkbox_skipfft4im,'Value');
gPrereadAllMDH = get(handles.checkbox_prereadAllMDH,'Value');
g3Dradial_ejaCODE = get(handles.checkbox_3Dradial_ejaCODE,'Value');
zeroPad_kz = get(handles.checkbox_zeroPad_kz,'Value');
% ----------------------------------------------

mdh_byte = 128; % measurement data header (byte) -> fixed
...
% slice mode and series mode needs to be investigated - 2010.11.02

if strcmp(image_spec.multiSliceMode,'MSM_INTERLEAVED') && useInterleaving
    % if even # of slices & ...interleaved ?
    if ~mod(image_spec.ns,2) && strcmp(image_spec.multiSliceSeriesMode,'INTERLEAVED')
...
file_size = ftell(fid);
disp(' ')
local_text4print = sprintf('File Size = %g MBytes\n',file_size/1e6);
fprintf(local_text4print)
disp2infoboard(info_fig_handle,' ')
disp2infoboard(info_fig_handle,local_text4print)
...
if ~reprocess
    ...
    disp2infoboard(info_fig_handle,'Checking saved raw...')
    ...
    if exist_full_raw_file
        ...
        fclose(fid_full_raw);
        local_text4print = strvcat('--------------------------------',...
            'Same version of processed full-raw data is exist.',...
            'Just save to seperate files.',...
            '--------------------------------',...
        ...
        disp('Done!')
        disp2infoboard(info_fig_handle,'Done!')
...
%         assignin('base','raw_info_text',raw_info_text)
        ...
        drawnow;
        save_RawimaComp(info_fig_handle)
        ...
        disp2infoboard(info_fig_handle,'=========================================================================')
        disp2infoboard(info_fig_handle,dispetime(toc,0))
        disp2infoboard(info_fig_handle,'=========================================================================')
        
        fclose(fid);
        clear raw raw_part0rg raw_part raw_part0rgAllCoil im
        return;
    end
...
%% <<< Procsessing some (or all) MDH and preallocating some variables>>>
...
if ~gPrereadAllMDH
    ...
    disp2infoboard(info_fig_handle,'----------------------------------')
    disp2infoboard(info_fig_handle,'Reading some MDH and preallocating some variables...')
    ...
    % --- readMDHmatsome(fid); %-- readMDHmat(fid) 랑 별 차이없음
    % --- 5~6 sec for 1G data
    % --- 그래도 readMDHmatsome 이 더 빠르다 - 2010.09.08
    readMDHmatsome(fid);
    
    while isempty(nx) && ftell(fid)<file_size*0.5
        ...
        if bitget(mMDH(MDH_aulEvalInfoMask1),MDH_SYNCDATA)
            ...
            cur_DMAlen = mMDH(MDH_ulDMALength);
            cur_DMAlen = bitand(uint32(cur_DMAlen), hex2dec('01FFFFFF'));
            fseek(fid,cur_DMAlen-mdh_byte,'cof');  %  -> byte ?
        else
        ...
        % ----- added at 2009.12.08
        % -- very faster than using 'mod'
        if processed_line == 2048 % mod(processed_line,1024)==0
            processed_line = 0;
            % ----------------- waitbar -----------------
        ...
        if cancel_process
            ...
            disp2infoboard(info_fig_handle,'Processing canceled.')
            fclose(fid);
            clear raw raw_part0rg raw_part raw_part0rgAllCoil im
            return;
        end
    ...
    disp('Some MDH read.')
    disp2infoboard(info_fig_handle,'Some MDH read.')
    
    fseek(fid,glob_hdr,'bof');    % skip (global hdr) from bof(begining of file)
    dispetime(clock,t1);
    disp2infoboard(info_fig_handle,dispetime(clock,t1))
    ...
    % estimate # of acq
    if ~isempty(nx)
        number_of_whole_acq = ceil(file_size/(nx*2*4));
        number_of_acq = ceil(number_of_whole_acq/nc);
    ...
    % % last_scan_cnt = mMDH(MDH_ulScanCounter)
    % %
    % % seg_indic = cell(last_scan_cnt,1);
    % % seg_indic_4s1 = cell(last_scan_cnt,1);
    % % acq_indic = cell(last_scan_cnt,1);
    % % whole_acq_indic = cell(last_scan_cnt,1);
    % % % ------------------------------------------------
    % %
    
else
    
    set(figHndl.text2,'string','Reading MDHs...');
    drawnow
    
    disp('----------------------------------')
    disp('Reading MDH and preallocating some variables...')
    disp2infoboard(info_fig_handle,'----------------------------------')
    disp2infoboard(info_fig_handle,'Reading MDH and preallocating some variables...')
    ...
    while ~bitget(mMDH(MDH_aulEvalInfoMask1),MDH_ACQEND)
        ...
        elseif bitget(mMDH(MDH_aulEvalInfoMask1),MDH_RTFEEDBACK)
            ...
            nx = cur_nx;
            nc = mMDH(MDH_ushUsedChannels);
            raw_ny = max(raw_ny,mMDH(MDH_sLC_ushLine));
            raw_nz = max(raw_nz,mMDH(MDH_sLC_ushPartition));
            
            if g3Dradial_ejaCODE
                raw_nida = max(raw_nida,mMDH(MDH_sLC_ushIda));
            end
            %-------mMDH------------------------------------
    ...
    disp('MDH read.')
    disp2infoboard(info_fig_handle,'MDH read.')
    ...
    if g3Dradial_ejaCODE;
        raw_nida = raw_nida +1;
        if raw_nida>1
            % allocate enough memory - cut residual after extract raw
            raw_ny = raw_nida*65535;
        end
    end
...
disp2infoboard(info_fig_handle,'--------------------------------')
disp2infoboard(info_fig_handle,'Using Standard I/O File acces.')
disp2infoboard(info_fig_handle,'MDH based recon')
...
% index_sync_data = 0; % -- removed at 2011.06.07
...
disp('Processing...')
disp2infoboard(info_fig_handle,'Processing...')
...
while ~bitget(mMDH(MDH_aulEvalInfoMask1),MDH_ACQEND)
    ...
    if bitget(mMDH(MDH_aulEvalInfoMask1),MDH_SYNCDATA)
        ...
        % --- read ADC data -> saved column vector
        % temp = fread(fid,cur_DMAlen-mdh_byte,'*char');  %
        temp = fread(fid,cur_DMAlen-mdh_byte,'uint8=>char');  % have an offset when directly using *char
        ...
        % ------------- add switch - 2010.06.01
        if ~donotPadZeros
            % --------------- modified at 2009.10.20 : combine below 2 method
            if image_spec.nx*2 == cur_nx
            ...
            % --------------------------------------------------------------
        end % --------------- end of donotPadZeros
        
        cur_preCut = mMDH(MDH_sCutOff_ushPre);
        cur_postCut = mMDH(MDH_sCutOff_ushPost);
        
        % replace zeros to ADC data % 윤호형 tse_rfrfx5 쓸때만 아래 두줄 주석처리!!
        ...
        if g3Dradial_ejaCODE
            cur_ida = mMDH(MDH_sLC_ushIda);
            cur_phi = [mMDH(MDH_aushIceProgramPara1);mMDH(MDH_aushIceProgramPara2)];
            cur_phi = typecast(uint16(cur_phi),'single');
            cur_theta = [mMDH(MDH_aushIceProgramPara3);mMDH(MDH_aushIceProgramPara4)];
            cur_theta = typecast(uint16(cur_theta),'single');
        end
        ...
        if bitget(mMDH(MDH_aulEvalInfoMask1),MDH_TREFFECTIVEBEGIN)...
            ...
            % modified at 2009.10.20
            if index_timeTReff>0 && ~isempty(timeTReff{index_timeTReff,1}) % begin - end
...
%         sync_data(cur_y,:,cur_z,cur_c,...
%             cur_s,cur_e,cur_r,cur_set) = temp;
%         index_sync_data = index_sync_data+1; % -- removed at 2011.06.07
        sync_data = temp;   % i found this flag in %AdjustSeq%/AdjCoilSensSeq
    ...
    elseif bitget(mMDH(MDH_aulEvalInfoMask1),MDH_RTFEEDBACK)
        ...
        switch image_spec.Nav_RFtype
            ...
            case '0x2'
                ...
                if cur_usedCh==cur_c && bitget(mMDH(MDH_aulEvalInfoMask1),MDH_PHASEFFT)
                    ...
                    % % % % % % % %             temp_navRT_image = SOS(temp_navRT_image).*exp(1i*angle(sum(temp_navRT_image,3)));
                    % % % % % % % %
                    ...
                    % ------- 2009.12.08 - find out - HARD CODDING
                    ...
                    % ----- added at 2009.12.08
                    % first zero-padding
                    if isempty(navRT_data_expand)
                        navRT_data_expand = zeros(size(navRT_data,1)+4,size(navRT_data,2),size(navRT_data,3),'single');
                    end
                    navRT_data_expand(3:14,:,:) = navRT_data;
                    
                    % ifft : spoil image (break rule) - found at 2010.09.08
                    % do FFT
                    temp_navRT_image = fft3c(navRT_data_expand,3);
                    
                    % %             % see each image
                    % %             mrimagef(SOS(temp_navRT_image).')
                    % %             axis normal
                    ...
                    %             temp_navRT_image = temp_navRT_image_mag.*exp(1i*temp_navRT_image_ph);
                    ...
                    % ----- modified at 2009.12.08
                    % save image to file when 200 image is collected (TRnav(150ms)*200 = 30s)
                    if navRT_image_index==(200)   % nav_nx/2
                        % add 0 to filenameSS
                        ...
                        disp2infoboard(info_fig_handle,['Saving ',num2str(navRT_image_file_index),...
                        ...
                        % ----- modified at 2009.12.08
                        % -- handle in case of 'nav train is' lengthen than 200
                        if ~nav_train_saved
        ...
        % seperate each coil to cell to avoid 'Out of Memory'
        % ----- added at 2009.12.08 'ignore_image_data'
        if ~ignore_image_data && cur_c ~= cp_coil_index    % did not include CP coil
            
            % not confirmed - 2010.09.09
            if g3Dradial_ejaCODE
                % unwrap cview, which wraps at 65535 (ushort) - from rdMeas_slt
                cur_y = cur_ida*65535 + cur_y; % cur_ida = 0,1, ..
                real_nymax = max(real_nymax,cur_y);
                
                % init angle matrix
                if isempty(angle_theta)
                    angle_theta = zeros(raw_ny,raw_nz,'single');
                    angle_phi = zeros(raw_ny,raw_nz,'single');
                end
                
                %  assume defined angle are all same for following indic
                if cur_c == cur_s == cur_e == cur_r == cur_set == cur_phase ==1
                    angle_theta(cur_y,cur_z) = cur_theta;
                    angle_phi(cur_y,cur_z) = cur_phi;
                end
            end
            ...
            if ~donotAvg && (cur_a>1 || mMDH(MDH_aushFreePara1) == 302) 
                ...
                if gPrereadAllMDH
                    % -------- dynamic allocation on matlab is very fast
                    % ---- thus, below does not improve speed much
                    try
                        if isempty(raw{cur_c,cur_s,1,cur_e,cur_r,cur_set,cur_phase});
                            raw{cur_c,cur_s,1,cur_e,cur_r,cur_set,cur_phase} = zeros(raw_ny,cur_nx,raw_nz,'single');
                        end
                    catch
                        raw{cur_c,cur_s,1,cur_e,cur_r,cur_set,cur_phase} = zeros(raw_ny,cur_nx,raw_nz,'single');
                    end
                ...
                if gPrereadAllMDH
                    ...
                    % ---- thus, below does not improve speed much
                    try
                        if isempty(raw{cur_c,cur_s,cur_a,cur_e,cur_r,cur_set,cur_phase});
                            raw{cur_c,cur_s,cur_a,cur_e,cur_r,cur_set,cur_phase} = zeros(raw_ny,cur_nx,raw_nz,'single');
                        end
                    catch
                        raw{cur_c,cur_s,cur_a,cur_e,cur_r,cur_set,cur_phase} = zeros(raw_ny,cur_nx,raw_nz,'single');
                    end
...
%             end
        ...
        % ----- added at 2009.12.08 'ignore_image_data'
        if ~ignore_image_data && mMDH(MDH_aushFreePara1) == 302
        ...
        if cur_c==1 && cur_a==1 && cur_r==1 && cur_s==1 && cur_phase==1
            if index_seg_indic_4s1>0 && is_triggered && is_navRT_image_prev && ~isempty(seg_indic_4s1)
                index_seg_indic_4s1 = index_seg_indic_4s1+1;
                seg_indic_4s1{index_seg_indic_4s1,1} = zeros(1,8);
            end
        ...
        if cur_c==1 && cur_a==1 && cur_r==1 && cur_phase==1
            if index_seg_indic>0 && is_triggered && is_navRT_image_prev && ~isempty(seg_indic)
                index_seg_indic = index_seg_indic+1;
                seg_indic{index_seg_indic,1} = zeros(1,9);
            end
            if cur_seg == 1 && ~is_triggered
                startTimeShot = cur_timeStamp;
            end
        ...
        if cur_c==1 && cur_r==1
            if index_acq_indic>0 && is_triggered && is_navRT_image_prev && ~isempty(acq_indic)
                index_acq_indic = index_acq_indic+1;
                acq_indic{index_acq_indic,1} = zeros(1,10);
            end
            if cur_seg == 1 && ~is_triggered
                startTimeShot = cur_timeStamp;
            end
        ...
        switch image_spec.Nav_RFtype
            ...
            case '0x2'
                ...
                if  ~isempty(navRT_image_trig_before_cell) && ~nav_train_saved% is_navRT_image_prev && size(navRT_image_trig_before_cell{1,1,1,1,1},2)==1
                    % ----- modified at 2009.12.08
                    % -- handle in case of 'nav train is' lengthen than 200
                    navRT_image_train = [navRT_image_train,navRT_image];
                    
                    if ~isdir([pathnameSS,'navRT_image'])
                        mkdir([pathnameSS,'navRT_image'])
                    end
                    disp2infoboard(info_fig_handle,['Saving ',...
    ...
    % ----- added at 2009.12.08
    % -- very faster than using 'mod' 
    if processed_line == 1024 % mod(processed_line,512)==0
        processed_line = 0;
        % ----------------- waitbar -----------------
        processed_byte = ftell(fid);
        processed_byte_percent = processed_byte/file_size*100;
...
disp2infoboard(info_fig_handle,'Extraction was done!')
...
if ~isempty(sync_data)
    ...
    fid_sync = fopen([pathnameSS,'sync_data/',filenameSS,'_','sync_data','.txt'],'w');
    fwrite(fid_sync, sync_data, 'char');
    fclose(fid_sync);
    clear sync_data
end
...
if ~isempty(timeTReff)
    ...
    local_text4print = sprintf('- average TReff = %g ms\n',mean(timeTReff(:,2)));
    fprintf(local_text4print)
    disp2infoboard(info_fig_handle,local_text4print)
...
if ~isempty(time_btwShot_Nav)
    ...
    local_text4print = sprintf('- average time between Shot & NAV = %g ms\n',mean(time_btwShot_Nav(:,2)));
    fprintf(local_text4print)
    disp2infoboard(info_fig_handle,local_text4print)
...
% ----- added at 2009.12.08 'ignore_image_data'
if ~ignore_image_data && ~isempty(projLine_raw_cell)
...
%% --------- save user data & cut residual size - 3D radial angle
if g3Dradial_ejaCODE
                ...
                for la = 1:na
                    for le = 1:ne
                        for lr = 1:nr
                            for lset = 1:nset
                                for lphase = 1:nphase
                                    raw{lc,ls,la,le,lr,lset,lphase}(real_nymax:end,:,:) = [];
    ...
    % save angle to file
    if ~isdir([pathnameSS,'angle'])
        mkdir([pathnameSS,'angle'])
    end
    save([pathnameSS,'angle/',filenameSS,'_','eja_CODE_angle','.mat'],'angle_theta','angle_phi');
    clear angle_theta angle_phi
    disp('angle saved to file.')
    disp2infoboard(info_fig_handle,'angle saved to file.')
end

%% Save raw and make image

% ----- added at 2009.12.08 'isempty(raw) || '
if isempty(raw) || isempty(raw{1})
    ...
    disp('------------------')
    disp2infoboard(info_fig_handle,'------------------')
    disp2infoboard(info_fig_handle,'No Image Data.')
    disp2infoboard(info_fig_handle,'Algorithm stopped.')
    disp2infoboard(info_fig_handle,'------------------')
else
%% averaging to acq index
    ...
    if ~donotAvg
        ...
        t1=clock;
        disp('Averaging...')
        disp2infoboard(info_fig_handle,'Averaging...')
                                ...
                                for lphase = 1:nphase                                    
                                    for ly = 1:ny
                                        for lz = 1:nz
                                            ...
                                            if ~isempty(raw{lc,ls,la,le,lr,lset,lphase}) && avg_cell{lc,ls,la,le,lr,lset,lphase}(ly,1,lz)>0
...
%% consider KyCenter & KzCenter
    ...
    disp2infoboard(info_fig_handle,'consider KyCenter & KzCenter...')
    ...
    if ~donotPadZeros
        ...
        % zero padding to locate CentreLine center
        if num_zero_padding>0
    ...
    end % --------------- end of zeroPad_kz

%% consider PE Resolution & slice Resolution

    disp('consider PE Resolution & slice Resolution...')
    disp2infoboard(info_fig_handle,'consider PE Resolution & slice Resolution...')
    
    % ---- get resolution
    RO_pixel_size = image_spec.FOVro*image_spec.read_os/nx;    
    PE_pixel_size_original = (image_spec.FOVpe*(image_spec.ny_res+image_spec.ny_os))/ny;
    Th_pixel_size_original = (image_spec.thick*(image_spec.nz_res+image_spec.nz_os))/nz;
    ...
    if ~donotPadZeros
        ...
        % zero padding to fit PE resolution
        if num_zero_padding>0
            index_y = index_y + num_zero_padding;
        ...
        if nz>1
            ...
            % zero padding to fit slice resolution
            if num_zero_padding>0
                index_z = index_z + num_zero_padding;
    ...
    PE_pixel_size = (image_spec.FOVpe*(image_spec.ny_res+image_spec.ny_os))/ny;
    Th_pixel_size = (image_spec.thick*(image_spec.nz_res+image_spec.nz_os))/nz;
...
%% save full raw data
    ...
    raw_info_text = strvcat(' ',...
        ...
        ' ',...
        'Final data size :',...
        sprintf('\timage (ky)x(kx)x(kz) : %d x %d x %d',ny,nx,nz),...
        ...
        raw_is_avged_text,...
        ' ',...
        sprintf('- image size\t(y, x, z) = %d x %d x %d',ny,nx,nz),...
        sprintf('- FOV \t(y, x, z) = %d x %d x %d mm',image_spec.FOVpe,image_spec.FOVro,image_spec.thick),...
        sprintf('- pixel size \t(y, x, z) = %.2f x %.2f x %.2f mm',PE_pixel_size,RO_pixel_size,Th_pixel_size),...
        sprintf('- resolution         :  PE = %.2f %%,   slice = %.2f %%',image_spec.ny_res*100,image_spec.nz_res*100),...
        sprintf('- oversampling    :  PE = %.2f %%,   slice = %.2f %%',image_spec.ny_os*100,image_spec.nz_os*100),...
        '*******************************************************',...
        sprintf('- fisrt te  & echo spacing [usec] :  fte = %.2f,   te = %.2f ',mMDH(MDH_aushFreePara1),mMDH(MDH_aushFreePara2)),...
        sprintf('- zshim momentum [mT/m * usec] :  1st zshim = %.2f,  2nd zshim = %.2f ',mMDH(MDH_aushFreePara3),mMDH(MDH_aushFreePara4)),...
        '*******************************************************',...
        '---------------------------------------------',...
        ' ');
    
    disp(raw_info_text)
    disp2infoboard(info_fig_handle,raw_info_text)
    ...
    if ~donotSaveFullRaw
        ...
        disp2infoboard(info_fig_handle,'Saving full-raw data to file...')
        drawnow
        ...
        if full_raw_FileSizeMBytes > 2000
            ...
            disp2infoboard(info_fig_handle,'Matlab can save file larger than 2GB in Version 7.3 or later')
            try
                ...
                try
                    ...
                    disp2infoboard(info_fig_handle,'Cannot save file.')
                end
            end
        else
            save(PathFIleName_of_saved_full_raw_file,...
        ...
        %---------------------------------------
        disp('Done!')
        dispetime(clock,t_save_start)
        disp(' ')
        disp2infoboard(info_fig_handle,'Done!')
        disp2infoboard(info_fig_handle,dispetime(clock,t_save_start))
        disp2infoboard(info_fig_handle,' ')
        
    else
        disp('Skip saving full-raw data to file.')
        disp2infoboard(info_fig_handle,'Skip saving full-raw data to file.')
        drawnow
    end % --------------- end of donotSaveFullRaw
...
disp('=========================================================================')
disp(['File Name : ',filenameSS]);
disp(['Total ',dispetime(toc,0)]);
disp('=========================================================================')
disp2infoboard(info_fig_handle,'=========================================================================')
disp2infoboard(info_fig_handle,['File Name : ',filenameSS]);
disp2infoboard(info_fig_handle,['Total ',dispetime(toc,0)]);
disp2infoboard(info_fig_handle,'=========================================================================')
...
%% function save_RawimaComp
    ...
    function save_RawimaComp(info_fig_handle)
        drawnow;
...
%% Seperate full-raw
        ...
        local_text4print = sprintf(' - Full Raw data size = %g [MBytes]\n',full_raw_FileSizeMBytes);
        fprintf(local_text4print)
        disp2infoboard(info_fig_handle,local_text4print)

        if gSkipifft4im
            do_separate_full_raw = 0;
        else
            %----------- check available physical memory
            try
                [user sys] = memory;
                available_phMEMSizeMBytes = sys.PhysicalMemory.Available/1e6;
                local_text4print = sprintf(' - Available physical memory : %g [MBytes]\n',available_phMEMSizeMBytes);
                fprintf(local_text4print)
                disp2infoboard(info_fig_handle,local_text4print)
            catch
                available_phMEMSizeMBytes = 8e3; % modified from 0 at 2010.06.10
                local_text4print = sprintf(' - function ''memory'' is not available.\n But assume sufficient memory.\n');
                fprintf(local_text4print)
                disp2infoboard(info_fig_handle,local_text4print)
            end
            ...
            local_text4print = sprintf(' - Needed memory size (raw, image, FFT): %g [MBytes]\n',needed_memory_sizeMBytes);
            fprintf(local_text4print)
            disp2infoboard(info_fig_handle,local_text4print)
        ...
        if do_separate_full_raw
            ...
            disp('Saving...')
            disp2infoboard(info_fig_handle,'Not enough Available physical memory.')
            disp2infoboard(info_fig_handle,'Process will save to seperate files.')
            disp2infoboard(info_fig_handle,'Saving...')
                                ...
                                for r = 1:nr
                                    ...
                                    for i_set = 1:nset
                                        ...
                                        for iphase = 1:nphase
                                            ...
                                            disp2infoboard(info_fig_handle,['Saving raw separately... ',...
                            ...
                            for i_set = 1:nset
                                ...
                                for iphase = 1:nphase
                                    ...
                                    if make_3DwithCoil
                                        ...
                                        for c = 1:nc
                                            ...
                                            if is_full_raw
                                                if ~isempty(raw{c,s,ia,e,r,i_set,iphase})
                                                    %-- add at 2009.12.06
                                                    cur_size_y = size(raw{c,s,ia,e,r,i_set,iphase},1);
                                                    cur_size_z = size(raw{c,s,ia,e,r,i_set,iphase},3);
                                                    
                                                    raw_part(index_y(1):index_y(1)+cur_size_y-1,:,index_z(1):index_z(1)+cur_size_z-1,c)...
                                                        = raw{c,s,ia,e,r,i_set,iphase};
                                                end
                                            else
                                                disp2infoboard(info_fig_handle,['Loading seperated raw... ',...
                                                    'raw_part0rg',mySiemensRead_version,...
                                                    '_a',add0a,num2str(ia),...
                                                    '_r',add0r,num2str(r),...
                                                    '_set',add0set,num2str(i_set),...
                                                ...
                                                if ~isempty(raw_part0rg)
                                                    %-- add at 2009.12.06
                                                    cur_size_y = size(raw_part0rg,1);
                                                    cur_size_z = size(raw_part0rg,3);
                                        ...
                                        disp2infoboard(info_fig_handle,['Saving s',add0s,num2str(s),'th, 3D raw (4D : 3D with coils) to file...'])
                                        %----- save raw to .mat file
                                        if ~isdir([pathnameSS,'Raw_Data_VB15'])
                                            mkdir([pathnameSS,'Raw_Data_VB15'])
                                        end
                                        ...
                                        if ~gSkipifft4im
                                            ...
                                            disp2infoboard(info_fig_handle,'Check available physical memory for FFT...')
                                            try
                                                [user sys] = memory;
                                                available_phMEMSizeMBytes = sys.PhysicalMemory.Available/1e6;
                                                local_text4print = sprintf(' - Available physical memory : %g [MBytes]\n',available_phMEMSizeMBytes);
                                                ...
                                                available_phMEMSizeMBytes = 0;
                                                local_text4print = sprintf(' - function ''memory'' is not available.\n');
                                                disp2infoboard(info_fig_handle,local_text4print)
                                            end
                                            
                                            needed_memory_sizeMBytes4FFT = needed_memory_sizeMBytes*2/3;
                                            local_text4print = sprintf(' - Needed memory size (image, FFT): %g [MBytes]\n',needed_memory_sizeMBytes4FFT);
                                            disp2infoboard(info_fig_handle,local_text4print)
                                            
                                            if available_phMEMSizeMBytes > needed_memory_sizeMBytes4FFT
                                                disp2infoboard(info_fig_handle,'Now, evaluate FFT...')
                                                im = fft3c(raw_part,11);
                                            else
                                                disp2infoboard(info_fig_handle,'Not enough memory for FFT.')
                                                clear raw_part
                                                ...
                                                for c = 1:nc
                                                    ...
                                                    raw_temp(index_y(1):index_y(1)+cur_size_y-1,:,index_z(1):index_z(1)+cur_size_z-1)...
                                                        = raw_part0rg;
                                                    
                                                    local_text4print = sprintf('Now, evaluate FFT of coil_%s%d...\n',add0c,c);
                                                    disp2infoboard(info_fig_handle,local_text4print)
                                                    im(:,:,:,c) = fft3c(raw_temp);
                                            ...
                                            disp2infoboard(info_fig_handle,['Saving s',add0s,num2str(s),'th, 3D image (4D : 3D with coils) to file...'])
                                        ...
                                        for c = 1:nc
                                            ...
                                            if is_full_raw
                                                if ~isempty(raw{c,s,ia,e,r,i_set,iphase})
                                                    ...
                                                    raw_part(index_y(1):index_y(1)+cur_size_y-1,:,index_z(1):index_z(1)+cur_size_z-1)...
                                                        = raw{c,s,ia,e,r,i_set,iphase};
                                                end
                                            else
                                                disp2infoboard(info_fig_handle,['Loading seperated raw... ',...
                                            ...
                                            disp2infoboard(info_fig_handle,['Saving s',add0s,num2str(s),'th, c',add0c,num2str(c),'th, 3D raw (3D without coils) to file...'])
                                            %----- save raw to .mat file
                                            if ~isdir([pathnameSS,'Raw_Data_VB15'])
                                                mkdir([pathnameSS,'Raw_Data_VB15'])
                                            end
                                            ...
                                            if ~gSkipifft4im
                                                ...
                                                disp2infoboard(info_fig_handle,['Saving s',add0s,num2str(s),'th, c',add0c,num2str(c),'th, 3D image (3D : 3D without coils) to file...'])
                                                %----- save image to .mat file
                                                if ~isdir([pathnameSS,'Reconstructed_Data_VB15'])
                                                    mkdir([pathnameSS,'Reconstructed_Data_VB15'])
                                                end
                        ...
                        for r = 1:nr
                            ...
                            for i_set = 1:nset
                                ...
                                for iphase = 1:nphase
                                    ...
                                    if is_full_raw
                                        for c=1:nc
                                            if ~isempty(raw{c,s,ia,e,r,i_set,iphase})
                                                ...
                                                raw_part(index_y(1):index_y(1)+cur_size_y-1,:,c)...
                                        ...
                                        for c=1:nc
                                            if ~isempty(raw_part0rgAllCoil{c})
                                                %-- add at 2009.12.06
                                                cur_size_y = size(raw_part0rgAllCoil{c},1);
                                    ...
                                    disp2infoboard(info_fig_handle,['Saving s',add0s,num2str(s),'th, raw to file...'])
                                    %----- save raw to .mat file
                                    if ~isdir([pathnameSS,'Raw_Data_VB15'])
                                        mkdir([pathnameSS,'Raw_Data_VB15'])
                                    end
                                    ...
                                    if ~gSkipifft4im
                                        ...
                                        disp2infoboard(info_fig_handle,['Saving s',add0s,num2str(s),'th, image to file...'])
                                        %----- save image to .mat file
                                        if ~isdir([pathnameSS,'Reconstructed_Data_VB15'])
                                            mkdir([pathnameSS,'Reconstructed_Data_VB15'])
                                        end
...
%% make & save composite image
        ...
        if make_comp_im
            ...
            try
                disp2infoboard(info_fig_handle,' ')
                func_make_comp_im(image_spec,raw_info_text,info_fig_handle,make_3DwithCoil)
                disp2infoboard(info_fig_handle,' ')
            catch ME
                disp(ME)
                disp(ME.stack(1))
...
%                     disp('-------------------------------------')
                ...
                set(figHndl.text_processed,'string','Error has occured..');
                disp2infoboard(info_fig_handle,' ')
...
function func_make_comp_im(image_spec,raw_info_text,info_fig_handle,make_3DwithCoil)
global cancel_process;
global image_spec_MDH;
                ...
                for r = 1:nr
                    ...
                    for e = 1:ne
                        ...
                        for s = 1:ns
                            ...
                            if make_3DwithCoil
                                disp2infoboard(info_fig_handle,'Loading  3D image (4D : 3D without coils) file...')
                                disp2infoboard(info_fig_handle,[' - ','image',mySiemensRead_version,...
                                    '_a',add0a,num2str(ia),...
                                    '_r',add0r,num2str(r),...
                                    '_set',add0set,num2str(i_set),...
                                    '_p',add0p,num2str(iphase),...
                                ...
                                for c = 1:nc
                                    ...
                                    disp2infoboard(info_fig_handle,['Loading ',num2str(c),'th coil 3D image file...'])
                                    disp2infoboard(info_fig_handle,[' - ','image',mySiemensRead_version,...
                                        '_a',add0a,num2str(ia),...
                                        '_r',add0r,num2str(r),...
                                        '_set',add0set,num2str(i_set),...
                            ...
                            comp_im = comp_mag_im.*exp(1i*comp_ph_im);
                            
                            disp2infoboard(info_fig_handle,'Saving composite image to file...')
                            disp2infoboard(info_fig_handle,[' - ','Composite_image',mySiemensRead_version,...
                                        '_a',add0a,num2str(ia),...
                                        '_r',add0r,num2str(r),...
                                        '_set',add0set,num2str(i_set),...
                                        '_p',add0p,num2str(iphase),...
                            ...
                            if cancel_process
                                disp('Processing canceled.')
                                disp2infoboard(info_fig_handle,'Processing canceled.')
                                clear comp_mag_im comp_ph_im comp_im
                                return;
                            end
                        end
                    end
                ...
                for r = 1:nr
                    ...
                    for e = 1:ne
                        ...
                        for s = 1:ns
                            ...
                            disp2infoboard(info_fig_handle,'Loading 2D image file...')
                            load([pathnameSS,'Reconstructed_Data_VB15/',filenameSS,...
                                '_image',mySiemensRead_version,...
                                '_a',add0a,num2str(ia),...
                                '_r',add0r,num2str(r),...
                            ...
                            comp_im = SOS(im).*exp(1i*angle(sum(im,3)));
                            clear im
                            
                            disp2infoboard(info_fig_handle,['Saving s',add0s,num2str(s),'th, composite image to file...'])
                            % ------ save combined image to .mat file
                            if ~isdir([pathnameSS,'Reconstructed_Data_VB15/Composite_image'])
                                mkdir([pathnameSS,'Reconstructed_Data_VB15/Composite_image'])
                            end
