...
disp('NOTE : All files will save compatible with Version 7.3 or later.')
...
%% +++++ Procsessing ONLY MDH mat ++++++
% 큰 파일에서는 dynamic memory allocation 이 더 빠름
...
% %         if cur_c ~= cp_coil_index    % did not include CP coil
%                         
%             cur_a = mMDH(MDH_sLC_ushAcquisition);  % just add if this >1
%             cur_e = mMDH(MDH_sLC_ushEcho);
%             cur_s = mMDH(MDH_sLC_ushSlice); % reorder slice
%             cur_seg = mMDH(MDH_sLC_ushSeg);
%             cur_z = mMDH(MDH_sLC_ushPartition);
%             cur_y = mMDH(MDH_sLC_ushLine);
%             cur_r = mMDH(MDH_sLC_ushRepetition);
%             cur_set = mMDH(MDH_sLC_ushSet);
%             % added at 2009.05.25
%             cur_phase = mMDH(MDH_sLC_ushPhase);
%             
%             %------- get larger index -----------------
%             ny = max(ny,cur_y);
%             nz = max(nz,cur_z);
...
% % nc
% 
% % {ns,nc,na,ne,nr,nset,nphase} [ny,nx,nz]
% t1=clock;
% %------ initialize raw matrix ----------------
% disp('initialize raw cell matrix...')
% if donotAvg
%     raw = cell(nc,ns,na,ne,nr,nset,nphase);
%     avg_cell = cell(nc,ns,na,ne,nr,nset,nphase);
%     raw1zeros = complex(zeros(ny,nx,nz,'single'));
%     avg1zeros = zeros(ny,1,nz,'single');
%     for lc=1:nc
%         for ls=1:ns
%             for la=1:na
%                 for le=1:ne
...
%                         for lset=1:nset
%                             for lphase=1:nphase
%                                 raw{lc,ls,la,le,lr,lset,lphase} = raw1zeros;
%                                 avg_cell{lc,ls,la,le,lr,lset,lphase} = avg1zeros;
%                             end
%                         end
%                     end
%                 end
...
%     end
% else
%     raw = cell(nc,ns,1,ne,nr,nset,nphase);
%     avg_cell = cell(nc,ns,1,ne,nr,nset,nphase);
%     raw1zeros = complex(zeros(ny,nx,nz,'single'));
%     avg1zeros = zeros(ny,1,nz,'single');    
%     for lc=1:nc
%         for ls=1:ns
...
%                     for lset=1:nset
%                         for lphase=1:nphase                            
%                             raw{lc,ls,1,le,lr,lset,lphase} = raw1zeros;
%                             avg_cell{lc,ls,1,le,lr,lset,lphase} = avg1zeros;
%                         end
%                     end
%                 end
%             end
...
%             temp_navRT_image = temp_navRT_image(nav_ny/2,:);
            ...
            if navRT_image_index==(nav_nx/2)
                ...
                save([pathnameSS,'navRT_image/',filenameSS,'_navRT_image_',add0,...
                    num2str(navRT_image_file_index),'.mat'],'navRT_image','-v7.3');
                disp('Done!')
                        ...
                        for ls = 1:ns
                            ...
                            if ~isempty(navRT_image_before_trig)
                                save([pathnameSS,'navRT_image/',filenameSS,'_navRT_image_before_trig',...
                                    ...
                                    '_e',add0e,num2str(le),...
                                    '_s',add0,num2str(ls),...
                                    '.mat'],'navRT_image_before_trig','-v7.3');
                            end
                        ...
                        for ls = 1:ns
                            ...
                            if ~isempty(navRT_image_after_trig)
                                save([pathnameSS,'navRT_image/',filenameSS,'_navRT_image_after_trig',...
                                    ...
                                    '_e',add0e,num2str(le),...
                                    '_s',add0,num2str(ls),...
                                    '.mat'],'navRT_image_after_trig','-v7.3');
                            end
...
if ~isempty(noise_data)
    ...
    save([pathnameSS,'noise_data/',filenameSS,'_','noise_data','.mat'],'noise_data','-v7.3');
    clear noise_data
end
...
if ~isempty(navHP_data)
    ...
    save([pathnameSS,'navHP_data/',filenameSS,'_','navHP_data','.mat'],'navHP_data','-v7.3');
    clear navHP_data
end
...
if ~isempty(refphstabscan_data)
    ...
    save([pathnameSS,'refphstabscan_data/',filenameSS,'_','refphstabscan_data','.mat'],'refphstabscan_data','-v7.3');
...
if ~isempty(phstabscan_data)
    ...
    save([pathnameSS,'noise_data/',filenameSS,'_','phstabscan_data','.mat'],'phstabscan_data','-v7.3');
...
if ~isempty(sync_data)
    ...
    save([pathnameSS,'sync_data/',filenameSS,'_','sync_data','.mat'],'sync_data','-v7.3');
    clear sync_data
end
...
if ~isempty(seg_indic)
    ...
    save([pathnameSS,'seg_indic/',filenameSS,'_','seg_indic','.mat'],...
        'seg_indic','seg_indic_info','seg_indic_info_4excel','-v7.3');
...
if ~isempty(seg_indic_4s1)
    ...
    save([pathnameSS,'seg_indic/',filenameSS,'_','seg_indic_4s1','.mat'],...
        'seg_indic_4s1','seg_indic_info_4s1','seg_indic_info_4s1_4excel','-v7.3');
    clear seg_indic_4s1
...
if ~isempty(acq_indic)
    ...
    save([pathnameSS,'seg_indic/',filenameSS,'_','acq_indic','.mat'],...
        'acq_indic','acq_indic_info','acq_indic_info_4excel','-v7.3');
    clear acq_indic
...
if ~isempty(whole_acq_indic)
    ...
    save([pathnameSS,'seg_indic/',filenameSS,'_','whole_acq_indic','.mat'],...
        'whole_acq_indic','whole_acq_indic_info','whole_acq_indic_info_4excel','-v7.3');
...
if ~isempty(timeTReff)
    ...
    save([pathnameSS,'Nav_time_Stamps/',filenameSS,'_','timeTReff','.mat'],...
        'timeTReff','timeTReff_info','timeTReff_info_4excel','-v7.3');
    fprintf('\n- average TReff = %g ms\n',mean(timeTReff(:,2)))
    clear timeTReff
...
if ~isempty(time_btwShot_Nav)
    ...
    save([pathnameSS,'Nav_time_Stamps/',filenameSS,'_','time_btwShot_Nav','.mat'],...
        'time_btwShot_Nav','time_btwShot_Nav_info','time_btwShot_Nav_info_4excel','-v7.3');
                        ...
                        for ls = 1:ns
                            ...
                            if ~isempty(projLine_raw)
                                ...
                                save([pathnameSS,'UserData/',filenameSS,'_','projLine_im',...
                                    ...
                                    '_s',add0,num2str(ls),...
                                    '.mat'],...
                                    'projLine_raw','projLine_im','projLine_im_SOS','-v7.3');%,'realCenterLine'
                            end
...
%% save full raw data
    ...
    raw_info_text = strvcat(' ',...
        ...
        '---------------------------------------------',...
        'full_raw data structure : {ns,nc,na,ne,nr,nset,nphase} [ny,nx,nz]',...
        ' ',...
...
%% seperate full-raw
        ...
        [user sys] = memory;
        available_phMEMSizeMByte = sys.PhysicalMemory.Available/1e6;        
        fprintf(' - Available physical memory : %g [MB]\n',available_phMEMSizeMByte)
        
        %---------- calculate needed memory
        if nz>1 % 3D data
            if make_3DwithCoil
                needed_memory_sizeMB = ny*nx*nz*nc*8*3/1e6;
            else
                needed_memory_sizeMB = ny*nx*nz*8*3/1e6;
            end   
        else % 2D data
            needed_memory_sizeMB = ny*nx*nc*8*3/1e6;
        end
        fprintf(' - Needed memory size (raw, image, FFT): %g [MB]\n',needed_memory_sizeMB)
        
        do_separate_full_raw = available_phMEMSizeMByte < needed_memory_sizeMB;
        disp(' ')
            ...
            if nz>1 % in 3D, to each coil
                for c=1:nc
                    ...
                    save([pathnameSS,'Raw_Data_VB15/',filenameSS,...
                        '_raw_coil_',add0c,num2str(c),mySiemensRead_version,'.mat'],'raw_coil','-v7.3');
                ...
                for s=1:ns
                    ...
                    save([pathnameSS,'Raw_Data_VB15/',filenameSS,...
                        '_raw_slice_',add0s,num2str(s),mySiemensRead_version,'.mat'],'raw_slice','-v7.3');
                            ...
                            for e = 1:ne
                                ...
                                for s = 1:ns
                                    ...
                                    if make_3DwithCoil
                                        ...
                                        for c = 1:nc
                                            ...
                                            if is_full_raw
                                                ...
                                                load([pathnameSS,'Raw_Data_VB15/',filenameSS,...
                                                    '_raw_coil_',add0c,num2str(c),mySiemensRead_version,'.mat'])
                                                
                                                if ~isempty(raw_coil{1,s,ia,e,r,i_set,iphase})
                                                    raw_part(index_y(1):index_y(2),:,index_z(1):index_z(2),c)...
                                                        = raw_coil{1,s,ia,e,r,i_set,iphase}(:,:,:);
                                                end
                                                clear raw_coil
                                        ...
                                        available_phMEMSizeMByte = sys.PhysicalMemory.Available/1e6;
                                        fprintf(' - Available physical memory : %g [MB]\n',available_phMEMSizeMByte)
                                        needed_memory_sizeMB = needed_memory_sizeMB*2/3;
                                        fprintf(' - Needed memory size (raw, image, FFT): %g [MB]\n',needed_memory_sizeMB)
                                        
                                        if available_phMEMSizeMByte > needed_memory_sizeMB
                                    ...
                                    if ~make_3DwithCoil % if 'make_3DwithCoil' unchecked
                                        ...
                                        for c = 1:nc
                                            ...
                                            save([pathnameSS,'Raw_Data_VB15/',filenameSS,...
                                                ...
                                                '_e',add0e,num2str(e),...
                                                '_s',add0s,num2str(s),...
                                                '_c',add0c,num2str(c),'.mat'],'raw_part','-v7.3');
                                            %---------------------------------------
                                            ...
                                            save([pathnameSS,'Reconstructed_Data_VB15/',filenameSS,...
                                                ...
                                                '_e',add0e,num2str(e),...
                                                '_s',add0s,num2str(s),...
                                                '_c',add0c,num2str(c),'.mat'],'im','-v7.3');
                                            %----------------------------------------
                                            disp('Done!')
                                            clear im
                                ...
                                for s = 1:ns
                                    ...
                                    if is_full_raw
                                        ...
                                        disp(['Loading seperated raw data file... ','raw_slice_',add0s,num2str(s)])
                                            load([pathnameSS,'Raw_Data_VB15/',filenameSS,...
                                                '_raw_slice_',add0s,num2str(s),mySiemensRead_version,'.mat'])
                                            
                                        for c=1:nc
                                            if ~isempty(raw_slice{c,1,ia,e,r,i_set,iphase})
                                                raw_part(index_y(1):index_y(2),:,c)...
                                                    = raw_slice{c,1,ia,e,r,i_set,iphase}(:,:,1);
                                            end
                                        end
                                        clear raw_slice
                                    end
