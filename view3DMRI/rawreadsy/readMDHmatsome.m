...
% fseek(fid,8,'cof');
% mMDH(3)                 = fread(fid, 1, 'uint32');      % 8 + 1*4 = 12
% fseek(fid,8,'cof');
...
% mMDH(8:17)                        = fread(fid, 10, 'uint16');      % 28 + 10*2 = 48
% fseek(fid,80,'cof');        % 48 + 80 = 128 OK!

%-- 8:17 까지 읽으나 별 시간차이 없음 - 2010.09.08
mMDH(8:27)                        = fread(fid, 20, 'uint16');      % 28 + 20*2 = 68
fseek(fid,60,'cof');        % 68 + 60 = 128 OK!

%-- 위랑 별 시간차이 없음
% mMDH(8:9)                        = fread(fid, 2, 'uint16');      % 28 + 2*2 = 32
% fseek(fid,96,'cof');        % 32 + 96 = 128 OK!
