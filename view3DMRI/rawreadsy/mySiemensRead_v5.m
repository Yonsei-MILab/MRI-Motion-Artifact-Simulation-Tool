...
%             temp_navRT_image = temp_navRT_image(nav_ny/2,:);
        ...
        % image raw data
        % structure : {nc,nz,ns,na} [ny,nx,ne,nr,nset,nphase]
    else
        ...
        if cur_c ~= cp_coil_index    % did not include CP coil
            if ~donotAvg && (cur_a>1 || sMDH.aushFreePara(1) == 302) 
                ...
                avg_cell{cur_c,cur_z,cur_s}(cur_y,1,...
                    cur_e,cur_r,cur_set,cur_phase) = avg_cell{cur_c,cur_z,cur_s}(cur_y,1,...
                    cur_e,cur_r,cur_set,cur_phase)+1;

                % add raw
                raw{cur_c,cur_z,cur_s}(cur_y,:,...
                    cur_e,cur_r,cur_set,cur_phase) = raw{cur_c,cur_z,cur_s}(cur_y,:,...
...
%                 realCenterLine(1,:,cur_c) = temp;
                ...
                avg_cell{cur_c,cur_z,cur_s,cur_a}(cur_y,1,...
                    cur_e,cur_r,cur_set,cur_phase) = 1;
                
                % save raw
                raw{cur_c,cur_z,cur_s,cur_a}(cur_y,:,...
                    cur_e,cur_r,cur_set,cur_phase) = temp;
            end
        end
...
if ~isempty(projLine_raw_cell)
    ...
    [nc,nz,ns,na] = size(raw);
    [ny,nx,ne,nr,nset,nphase] = size(raw{1});
                            ...
                            for lset = 1:nset
                                for lphase = 1:nphase
                                    for la = 1:na
                                        ...
                                        if ~isempty(raw{lc,lz,ls,la})
                                            raw{lc,lz,ls,la}(ly,:,...
                                                le,lr,lset,lphase) = raw{lc,lz,ls,la}(ly,:,...
                                                le,lr,lset,lphase)/avg_cell{lc,lz,ls,la}(ly,1,...
                                                le,lr,lset,lphase);
                                        end
...
%% consider KyCenter & KzCenter
    ...
    if num_zero_padding>0
        % seperate each coil to cell to avoid 'Out of Memory'
        temp_zeros = zeros(num_zero_padding,nx,ne,nr,nset,nphase,'single');
        for oc=1:nc
            for oz=1:nz
                for os=1:ns
                    for oa=1:na
                        if ~isempty(raw{oc,oz,os,oa})
                            raw{oc,oz,os,oa} = cat(1,temp_zeros,raw{oc,oz,os,oa});
                        end
                    end
                end
            end
        ...
        for oc=1:nc
            for oz=1:nz
                for os=1:ns
                    for oa=1:na
                        if ~isempty(raw{oc,oz,os,oa})
                            raw{oc,oz,os,oa} = cat(1,raw{oc,oz,os,oa},temp_zeros);
                        end
                    end
                end
            end
    ...
    if nz>1
        ...
        if num_zero_padding>0
            % seperate each coil to cell to avoid 'Out of Memory'
            temp_zeros = zeros(ny,nx,ne,nr,nset,nphase,'single');
            temp_zeros_cell = cell(nc,num_zero_padding,ns,na);
            ...
            for oc=1:nc
                for oz=1:num_zero_padding
                    for os=1:ns
                        for oa=1:na
                            if ~isempty(raw{oc,oz,os,oa})
                                temp_zeros_cell{oc,oz,os,oa} = temp_zeros;
                            end
                        end
                    end
                end
        ...
        for oc=1:nc
            for oz=1:nz
                for os=1:ns
                    for oa=1:na
                        if ~isempty(raw{oc,oz,os,oa})
                            raw{oc,oz,os,oa} = permute(raw{oc,oz,os,oa},[2,1,3,4,5,6]);
                        end
                    end
                end
            end
...
%% save full raw data
    ...
    save([pathnameSS,'Raw_Data_VB15/',filenameSS,...
        '_full_raw',flag_avged_full_raw,'.mat'],'raw','image_spec_MDH','image_spec','raw_info_text');
    %---------------------------------------
    disp('Done!')
    disp(' ')
                                ...
                                for s = 1:ns
                                    ...
                                    for c = 1:nc
                                        ...
                                        if is_full_raw
                                            ...
                                            % Memory'
                                            for z=1:nz
                                                if ~isempty(raw{c,z,s,ia})
                                                    raw_part(:,:,z) = raw{c,z,s,ia}(:,:,e,r,i_set,iphase);
                                                else
                                            ...
                                            for z=1:nz
                                                if ~isempty(raw_coil{1,z,s,ia})
                                                    raw_part(:,:,z) = raw_coil{1,z,s,ia}(:,:,e,r,i_set,iphase);
                                                else
                                                    raw_part(:,:,z) = 0;
                                                end
                                            end
                            ...
                            for e = 1:ne
                                ...
                                for s = 1:ns
                                    ...
                                    if is_full_raw
                                        % seperate each coil to cell to avoid 'Out of Memory'
                                        for c=1:nc
                                            if ~isempty(raw{c,1,s,ia})
                                                raw_part(:,:,c) = raw{c,1,s,ia}(:,:,e,r,i_set,iphase);
                                            else
                                                raw_part(:,:,c) = 0;
                                            end
                                        end
                                        ...
                                        for c=1:nc
                                            if ~isempty(raw_slice{c,1,1,ia})
                                                raw_part(:,:,c) = raw_slice{c,1,1,ia}(:,:,e,r,i_set,iphase);
                                            else
                                                raw_part(:,:,c) = 0;
                                            end
                                        end
